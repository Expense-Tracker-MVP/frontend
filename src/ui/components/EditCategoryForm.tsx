...
import { updateCategoryApi } from '@ui/lib/apis/categories'
import type { EditCategoryFormProps } from '@ui/lib/types/components'
...
const EditCategoryForm: React.FC<EditCategoryFormProps> = ({ category, onSuccess, onCancel }) => {
    ...
    const handleSubmit = async (e: React.FormEvent) => {
        ...
        setLoading(true)
        try {
            await updateCategoryApi(category.id!, { ...category, ...form })
            onSuccess?.()
        } catch (err: any) {
    ...
    return (
        <form onSubmit={handleSubmit} className="p-4 bg-surface rounded shadow">
            <h3 className="text-lg font-medium mb-3">Edit Category</h3>

            {error && (
                <div className="mb-3 text-sm text-red-700 bg-red-100 p-2 rounded dark:bg-red-900 dark:text-red-200">
                    {error}
                </div>
            )}
            ...
            <div className="flex items-center gap-2">
                ...
                <button
                    type="button"
                    onClick={onCancel}
                    className="px-3 py-2 border border-border rounded dark:text-slate-100"
                >
                    Cancel
                </button>
            </div>
